...
class CategoryController extends Controller
{
    ...
    public function lists(Request $request) {
        ...
        return view('admin.goods.category.lists', ['html' => $html]);
    }
    ...
    public function getTableHtml($category, $loop = 0) {
        ...
        if ($category) {
            foreach ($category as $val) {
                ...
                $image = $val['image'] ? '<img src="' . $val['image'] . '" width="30" height="30">' : '';
                $html .= '<tr id="row_category_id' . $val['id'] . '" class="category_id' . $val['parent_id'] . '" data-id="' . $val['id'] . '">';
                $html .= '<td>' . $pre . $val['title'] . $image . '</td>';
                $html .= '<td align="center">' . $val['position'] . '</td>';
                $html .= '<td align="center"><input type="checkbox" name="sex" value="' . $val['id'] . '" lay-skin="switch" lay-text="' . Category::STATUS_DESC[Category::STATUS_ON] . '|' . Category::STATUS_DESC[Category::STATUS_ON] . '" lay-filter="status_btn" ' . ($val['status'] == Category::STATUS_ON ? 'checked' : '') . '></td>';
                $html .= '<td>';
                if ($loop < Category::LOOP_LEVEL) {
    ...
    public function edit(Request $request) {
        if ($request->isMethod('post')) {
            ...
            if ($request->id) {
                $res = Category::where('id', $request->id)->update($save_data);
            } else {
                $result = Category::create($save_data);
                $res = $result->id;
            }
            if ($res) {
                return res_success();
            ...
            return view('admin.goods.category.edit', ['item' => $item]);
        }
    }
    ...
    public function status(Request $request) {
        ...
        if ($id && isset($status)) {
            $res = Category::where('id', $id)->update(['status' => $status]);
            if ($res) {
                return res_success();
            } else {
