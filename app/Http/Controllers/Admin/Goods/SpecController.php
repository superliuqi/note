...
class SpecController extends Controller
{
    ...
    public function lists(Request $request) {
        ...
        return view('admin.goods.spec.lists', ['category' => $category]);
    }
    ...
    public function listsAjax(Request $request) {
        ...
        $result = Spec::select('id', 'title', 'category_id', 'note', 'position', 'created_at')
            ->where($where)
            ->orderBy('id', 'desc')
    ...
    public function edit(Request $request) {
        if ($request->isMethod('post')) {
            ...
            if ($request->id) {
                $res = Spec::where('id', $request->id)->update($save_data);
            } else {
            ...
            //查询分类
            $category = Category::getSelectCategory();
            return view('admin.goods.spec.edit', ['item' => $item, 'category' => $category]);
        }
    }
    ...
    public function status(Request $request) {
        ...
        if ($ids && isset($status)) {
            $res = Spec::whereIn('id', $ids)->update(['status' => $status]);
            if ($res) {
                return res_success();
            } else {
    ...
    public function position(Request $request) {
        ...
        if ($id && isset($position)) {
            $res = Spec::where('id', $id)->update(['position' => $position]);
            if ($res) {
                return res_success();
            } else {
                return res_error('操作失败');
