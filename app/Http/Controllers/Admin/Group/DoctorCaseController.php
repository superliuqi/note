...
class DoctorCaseController extends Controller
{
    ...
    public function lists(Request $request) {
        ...
        return view('admin.group.doctorcase.lists',['doctor_id' => $doctor_id]);
    }
    ...
    public function listsAjax(Request $request) {
        ...
        $result = DoctorCase::select('id','image','contrast_image', 'created_at')
            ->where($where)
            ->orderBy('id', 'desc')
    ...
    public function edit(Request $request) {
        if ($request->isMethod('post')) {
            ...
            $validator = Validator::make($request->all(), [
                'image'=>'required',
                'contrast_image'=>'required',
            ], [
                'image.required' => '操作前图不能为空',
                'contrast_image.required' => '操作后图不能为空',
            ]);
            ...
            $save_data = array();
            foreach ($request->only(['image','contrast_image','doctor_id']) as $key => $value) {
            ...
            try {
                $res = DB::transaction(function () use ($request, $save_data) {
                    if ($request->id) {
                        $res = DoctorCase::where('id', $request->id)->update($save_data);
                    } else {
                        $result = DoctorCase::create($save_data);
                        $res = $result->id;
                    }
                    return $res;
                });
            ...
            $item = array();
            if ($request->id) {
                $item = DoctorCase::find($request->id);
                if (!$item) {
            ...
            return view('admin.group.doctorcase.edit', ['item' => $item]);
        }
    }
    ...
    public function delete(Request $request) {
        ...
        try {
            $res = DB::transaction(function () use ($ids) {
                $res = DoctorCase::whereIn('id', $ids)->delete();
                return $res;
            });
        } catch (\Exception $e) {
            $res = false;
