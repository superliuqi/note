...
use App\Models\YuBook;
use App\Models\YuBookPhoto;
 ...
 */
class YuBookController extends Controller
{
    /**
     * 列表
    ...
    public function lists(Request $request) {
        return view('admin.president.yubook.lists');
    }
    ...
    public function listsAjax(Request $request) {
        ...
        $result = YuBook::select('id', 'title', 'image', 'position', 'created_at', 'status')
            ->where($where)
            ->orderBy('id', 'desc')
    ...
    public function edit(Request $request) {
        if ($request->isMethod('post')) {
            ...
            $save_data = array();
            foreach ($request->only(['title', 'image','url','lang','position']) as $key => $value) {
            ...
            try {
                $res = DB::transaction(function () use ($request, $save_data) {
                    if ($request->id) {
                        $res = YuBook::where('id', $request->id)->update($save_data);
                        if($request->images){
                            YuBookPhoto::where('Book_id',$request->id)->delete();
                            foreach ($request->images as $key=>$v){
                                ...
                                YuBookPhoto::create($image_list);
                            }
                        }

                    } else {
                        $result = YuBook::create($save_data);
                        $res = $result->id;
                        if($request->images){
                            foreach ($request->images as $key=>$v){
                                $image_list = array();
            ...
            return view('admin.president.yubook.edit', ['item' => $item]);
        }
    }
    ...
    public function status(Request $request) {
        ...
        if ($ids && isset($status)) {
            $res = YuBook::whereIn('id', $ids)->update(['status' => $status]);
            if ($res) {
                return res_success();
            } else {
    ...
    public function delete(Request $request) {
        ...
        try {
            $res = DB::transaction(function () use ($ids) {
                $res = YuBook::whereIn('id', $ids)->delete();
                return $res;
            });
        } catch (\Exception $e) {
            $res = false;
    ...
    public function position(Request $request) {
        ...
        if ($id && isset($position)) {
            $res = YuBook::where('id', $id)->update(['position' => $position]);
            if ($res) {
                return res_success();
            } else {
                return res_error('操作失败');
