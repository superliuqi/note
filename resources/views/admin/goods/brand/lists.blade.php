...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('brand/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            <th lay-data="{type:'checkbox'}"></th>
            <th lay-data="{field:'id', width:80, sort: true}">ID</th>
            <th lay-data="{field:'title', width:200, toolbar: '#imgTpl'}">名称</th>
    ...
    <script type="text/html" id="imgTpl">
        @{{ d.title }}
        @{{# if(d.logo != ''){ }}
        <a href="@{{ d.logo }}" target="_blank"><img src="@{{ d.logo }}" width="30"></a>
        @{{# } }}
    </script>
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\Brand::STATUS_DESC[\App\Models\Brand::STATUS_ON] }}|{{ \App\Models\Brand::STATUS_DESC[\App\Models\Brand::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('brand/add') }}');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Brand::STATUS_ON }}};
                    global.footer_ajax('{{ url('brand/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Brand::STATUS_OFF }}};
                    global.footer_ajax('{{ url('brand/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('brand/delete') }}', post_data);
                    });
                }
            };
