...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('delivery/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            <th lay-data="{type:'checkbox'}"></th>
            <th lay-data="{field:'id', width:80, sort: true}">ID</th>
            <th lay-data="{field:'title', width:200}">名称</th>
            <th lay-data="{field:'open_default', width:100, align:'center', toolbar: '#open_defaultTpl'}">默认运费</th>
            <th lay-data="{field:'price_type', width:100, align:'center', toolbar: '#price_typeTpl'}">费用类型</th>
            <th lay-data="{field:'desc', minWidth:200}">描述</th>
    ...
    <script type="text/html" id="open_defaultTpl">

        <span class="layui-badge @{{# if (d.open_default == '启用'){ }}layui-bg-green@{{# }else{ }}layui-bg-orange@{{# } }}">@{{ d.open_default }}</span>
    </script>
    <script type="text/html" id="price_typeTpl">
        <span class="layui-badge @{{# if (d.price_type == '全国'){ }}layui-bg-green@{{# }else{ }}layui-bg-orange@{{# } }}">@{{ d.price_type }}</span>
    </script>
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\Delivery::STATUS_DESC[\App\Models\Delivery::STATUS_ON] }}|{{ \App\Models\Delivery::STATUS_DESC[\App\Models\Delivery::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('delivery/add') }}', '100%');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Delivery::STATUS_ON }}};
                    global.footer_ajax('{{ url('delivery/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Delivery::STATUS_OFF }}};
                    global.footer_ajax('{{ url('delivery/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('delivery/delete') }}', post_data);
                    });
                }
            };
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('delivery/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
                } else if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('delivery/edit') }}?id=' + data.id, '100%');
                }
            });
            //监听锁定操作
            form.on('switch(status_btn)', function(obj){
                var post_data = {_token: csrf_token, id:this.value, status:obj.elem.checked == true ? 1 : 0};
                global.footer_ajax('{{ url('delivery/status') }}', post_data, false);
            });
        });
    </script>
@endsection
