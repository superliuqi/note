...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('spec/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script type="text/html" id="actionButton">
        <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="add_value">规格值列表</a>
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加
                add: function(){
                    global.layer_show('添加', '{{ url('spec/add') }}');
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        var post_data = {_token: csrf_token};
                        global.footer_ajax('{{ url('spec/delete') }}', post_data);
                    });
                }
            };
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('spec/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
                } else if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('spec/edit') }}?id=' + data.id);
                } else if(obj.event === 'add_value'){
                    global.layer_show('规格值列表', '{{ url('spec_value') }}?spec_id=' + data.id, '100%');
                }
            });
            ...
            table.on('edit(row_list)', function(obj){
                ...
                global.footer_ajax('{{ url('spec/position') }}', post_data, false);
            });
        });
    </script>
@endsection
