...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('spec_value/lists_ajax') }}?spec_id={{ $spec_id }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('spec_value/add') }}?spec_id={{ $spec_id }}');
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        var post_data = {_token: csrf_token};
                        global.footer_ajax('{{ url('spec_value/delete') }}', post_data);
                    });
                }
            };
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('spec_value/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
                } else if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('spec_value/edit') }}?id=' + data.id);
                }
            });
            ...
            table.on('edit(row_list)', function(obj){
                ...
                global.footer_ajax('{{ url('spec_value/position') }}', post_data, false);
            });
        });
    </script>
@endsection
