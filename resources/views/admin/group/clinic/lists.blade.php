...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('clinic/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            <th lay-data="{type:'checkbox'}"></th>
            <th lay-data="{field:'id', width:80, sort: true}">ID</th>
            <th lay-data="{field:'title', width:250, toolbar: '#imgTpl'}">标题</th>
            <th lay-data="{field:'desc'}">简介</th>
            <th lay-data="{field:'level', width:70}">评分</th>
    ...
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\Clinic::STATUS_DESC[\App\Models\Clinic::STATUS_ON] }}|{{ \App\Models\Clinic::STATUS_DESC[\App\Models\Clinic::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加
                add: function(){
                    global.layer_show('添加', '{{ url('clinic/add') }}', '100%');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Clinic::STATUS_ON }}};
                    global.footer_ajax('{{ url('clinic/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Clinic::STATUS_OFF }}};
                    global.footer_ajax('{{ url('clinic/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('clinic/delete') }}', post_data);
                    });
                }
            };
