...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('clinic_city/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加
                add: function(){
                    global.layer_show('添加', '{{ url('clinic_city/add') }}', '100%');
                },
                ...
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('clinic_city/delete') }}', post_data);
                    });
                }
            };
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('clinic_city/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
                } else if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('clinic_city/edit') }}?id=' + data.id, '100%');
                }
            });
            ...
            table.on('edit(row_list)', function(obj){
                ...
                global.footer_ajax('{{ url('clinic_city/position') }}', post_data, false);
            });
        });
    </script>
@endsection
