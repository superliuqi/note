...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('doctor_video/lists_ajax') }}?doctor_id={{ $doctor_id }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            <th lay-data="{type:'checkbox'}"></th>
            <th lay-data="{field:'id', width:80, sort: true}">ID</th>
            <th lay-data="{field:'', toolbar: '#imgTpl'}">封面图</th>
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加
                add: function(){
                    global.layer_show('添加', '{{ url('doctor_video/add') }}?doctor_id={{ $doctor_id }}', '100%');
                },
                ...
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('doctor_video/delete') }}', post_data);
                    });
                }
            };
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('doctor_video/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
                } else if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('doctor_video/edit') }}?id=' + data.id, '100%');
                }
            });
