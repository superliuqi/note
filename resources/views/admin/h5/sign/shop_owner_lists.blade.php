...
@section('content')
    ...
    <div class="layui-form searchButton">
        <div class="search_input">
            <div class="layui-inline">
                <input class="layui-input" name="keyword" autocomplete="off" placeholder="店家名称">
            </div>
            <div class="layui-inline">
    ...
    <table class="layui-table" lay-data="{url:'{{ url('sign/shop_owner_lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            ...
            <th lay-data="{field:'chief_name'}">课长</th>
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('sign/shop_owner_add') }}','100%');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Gift::STATUS_ON }}};
                    global.footer_ajax('{{ url('sign/shop_owner_status') }}', post_data);
                },
                //锁定
                status_off: function(){
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('sign/shop_owner_delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                            layer.msg('删除成功');
                        }
