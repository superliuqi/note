...
@section('content')
    <div class="layui-form searchButton" style="margin-top: 8px;">
        <button class="layui-btn layui-btn-sm add_point" data-type="add"><i class="layui-icon">&#xe608;</i>充值美币</button>
        <button class="layui-btn layui-btn-sm del_point" data-type="reduce"><i class="layui-icon">&#xe640;</i>扣除美币</button>
        <div class="search_input">
            <div class="layui-input-inline">
                <select name="event" id="event">
                    <option value="">类型选择</option>
                    @foreach(\App\Models\MeibiDetail::EVENT_DESC as $ek=>$ev)
    ...
    <table class="layui-table" lay-data="{url:'{{ url('account/meibi_detail_ajax') }}?m_id={{ $m_id }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            ...
            $('.add_point,.del_point').on('click', function(index){
                ...
                layer.prompt(function(val, index){
                    if(check_money(val)){
                        ...
                        if(data_type === 'add'){
                            global.footer_ajax('{{ url('account/meibi_add') }}', post_data, true);
                        }else if (data_type === 'reduce'){
                            global.footer_ajax('{{ url('account/meibi_reduce') }}', post_data, true);
                        }
                        layer.close(index);
                    }else{
                        layer.msg('请输入正确的金额');
