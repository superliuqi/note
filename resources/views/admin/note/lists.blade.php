...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('note/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\Note::STATUS_DESC[\App\Models\Article::STATUS_ON] }}|{{ \App\Models\Note::STATUS_DESC[\App\Models\Article::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加
                add: function(){
                    global.layer_show('添加', '{{ url('note/add') }}', '100%');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Article::STATUS_ON }}};
                    global.footer_ajax('{{ url('note/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                ...
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('note/delete') }}', post_data);
                    });
                }
            };
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('note/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
                } else if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('note/edit') }}?id=' + data.id, '100%');
                }else if(obj.event === 'push'){
                    layer.confirm('确定推送吗', function(index){
                        var post_data = {_token: csrf_token, id: data.id};
                        if (global.footer_ajax('{{ url('note/push') }}', post_data, true)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
            ...
            form.on('switch(status_btn)', function(obj){
                ...
                global.footer_ajax('{{ url('note/status') }}', post_data, false);
            });

            //监听单元格编辑
            table.on('edit(row_list)', function(obj){
                var post_data = {_token: csrf_token, id: obj.data.id, position: obj.value};
                global.footer_ajax('{{ url('note/position') }}', post_data, false);
            });
        });
    </script>
@endsection
