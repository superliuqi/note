...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('yualbum/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加
                add: function(){
                    global.layer_show('添加', '{{ url('yualbum/add') }}', '100%');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\YuAlbum::STATUS_ON }}};
                    global.footer_ajax('{{ url('yualbum/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\YuAlbum::STATUS_OFF }}};
                    global.footer_ajax('{{ url('yualbum/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('yualbum/delete') }}', post_data);
                    });
                }
            };
