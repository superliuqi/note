...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('yubook/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\YuBook::STATUS_DESC[\App\Models\YuBook::STATUS_ON] }}|{{ \App\Models\YuBook::STATUS_DESC[\App\Models\YuBook::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加
                add: function(){
                    global.layer_show('添加', '{{ url('yubook/add') }}', '100%');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\YuBook::STATUS_ON }}};
                    global.footer_ajax('{{ url('yubook/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\YuBook::STATUS_OFF }}};
                    global.footer_ajax('{{ url('yubook/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('yubook/delete') }}', post_data);
                    });
                }
            };
