...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('admin_user/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\AdminUser::STATUS_DESC[\App\Models\AdminUser::STATUS_ON] }}|{{ \App\Models\AdminUser::STATUS_DESC[\App\Models\AdminUser::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('admin_user/add') }}');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\AdminUser::STATUS_ON }}};
                    global.footer_ajax('{{ url('admin_user/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\AdminUser::STATUS_OFF }}};
                    global.footer_ajax('{{ url('admin_user/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('admin_user/delete') }}', post_data);
                    });
                }
            };
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('admin_user/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
                } else if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('admin_user/edit') }}?id=' + data.id);
                }
            });
            //监听锁定操作
            form.on('switch(status_btn)', function(obj){
                var post_data = {_token: csrf_token, id:this.value, status:obj.elem.checked == true ? 1 : 0};
                global.footer_ajax('{{ url('admin_user/status') }}', post_data, false);
            });
        });
    </script>
@endsection
