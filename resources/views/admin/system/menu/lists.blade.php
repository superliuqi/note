...
@section('content')
    ...
    <div class="searchButton" style="padding: 0 10px;">
    ...
    <script>
        layui.config({
            ...
            var search_active = {
                ...
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('menu/add') }}');
                }
            };
            ...
            $('.layui-table .iconfont').on('click', function (){
                $('.menu_id' + $(this).attr('data-id')).toggle();
                //动态更换图标
            ...
            form.on('switch(status_btn)', function(obj){
                ...
                global.footer_ajax('{{ url('menu/status') }}', post_data, false);
            });
            ...
            $('.layui-table .layui-btn').on('click', function (){
                ...
                if (event == 'add_menu'){
                    global.layer_show('添加', '{{ url('menu/add') }}?parent_id=' + id);
                } else if(event == 'edit') {
                    global.layer_show('编辑', '{{ url('menu/edit') }}?id=' + id);
                } else if(event == 'del') {
                    layer.confirm('确定删除吗', function(index){
                        var post_data = {_token: csrf_token, id: id};
                        if (global.footer_ajax('{{ url('menu/delete') }}', post_data, false)) {
                            $('#row_menu_id' + id).remove();
                            layer.close(index);
                        }
                    });
                }
