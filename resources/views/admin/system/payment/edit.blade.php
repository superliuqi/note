    ...
    <div style="margin: 15px;">
        <form class="layui-form" method="post">
            ...
            <div class="layui-form-item">
                ...
                <input type="hidden" value="" name="logo" lay-verify="required" lay-errormsg="logo不能为空">
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label">描述</label>
                <div class="layui-input-block">
                    <input type="text" name="desc" lay-verify="required" lay-errormsg="描述不能为空" autocomplete="off" class="layui-input">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label">类型</label>
                <div class="layui-input-block">
                    @foreach(\App\Models\Payment::TYPE_DESC as $key => $value)
                        <input type="radio" name="type" value="{{ $key }}" title="{{ $value }}" lay-verify="required" @if ($key == \App\Models\Payment::TYPE_LINE) checked @endif>
                    @endforeach
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    @foreach(\App\Models\Payment::CLIENT_TYPE_DESC as $key => $value)
                        <input type="radio" name="client_type" value="{{ $key }}" title="{{ $value }}" lay-verify="required" @if ($key == \App\Models\Payment::CLIENT_TYPE_WEB) checked @endif>
                    @endforeach
                </div>
            </div>
            <div class="layui-form-item">
