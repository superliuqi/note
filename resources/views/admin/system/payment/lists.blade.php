...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('payment/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            <th lay-data="{type:'checkbox'}"></th>
            <th lay-data="{field:'id', width:80, sort: true}">ID</th>
            <th lay-data="{field:'title', width:170, toolbar: '#imgTpl'}">支付方式</th>
            <th lay-data="{field:'desc'}">描述</th>
            <th lay-data="{field:'position', width:80, align:'center', sort: true, edit: 'text'}">排序</th>
            <th lay-data="{field:'client_type', width:100, align: 'center'}">客户端类型</th>
    ...
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\Payment::STATUS_DESC[\App\Models\Payment::STATUS_ON] }}|{{ \App\Models\Payment::STATUS_DESC[\App\Models\Payment::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('payment/add') }}');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\ExpressCompany::STATUS_ON }}};
                    global.footer_ajax('{{ url('payment/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                ...
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('payment/delete') }}', post_data);
                    });
                }
            };
