    ...
    <div class="layui-form searchButton">
        ...
        <div class="search_input">
            ...
            <div class="layui-inline">
                <select name="os">
                    <option value=""></option>
                    @foreach(\App\Models\AppDownLog::SYSTEM_DESC as $key => $value)
...
@section('footer')
    ...
    <script>
        layui.config({
            ...
            var search_active = {
                ...
                search: function () {
                    ...
                    container(where_search['os'], where_search['start_time'], where_search['end_time'])
                }
            };
        ...
        function container(os, start_time, end_time) {
            var chart = $("#container").highcharts();
            // 获取 CSV 数据并初始化图表
            $.getJSON("{{ url('statistics/lists_ajax') }}" + "?os=" + os + "&start_time=" + start_time + "&end_time=" + end_time, function (csv) {
                var newdate = CSVToarray(csv, '');
                var data0 = new Array();
                var data1 = new Array();
                var data2 = new Array();
                var data3 = new Array();
                ...
                for (var i = 0; i < newdate.length; i++) {
                    data0[i] = newdate[i][0];
                    data1[i] = parseInt(newdate[i][1]);
                    data2[i] = parseInt(newdate[i][2]);
                    data3[i] = parseInt(newdate[i][3]);
                }
                data0.shift();
                data1.shift();
                data2.shift();
