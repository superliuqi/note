...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('adv/lists_ajax') }}?group_id={{ $group_id }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\Adv::STATUS_DESC[\App\Models\Adv::STATUS_ON] }}|{{ \App\Models\Adv::STATUS_DESC[\App\Models\Adv::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('adv/add') }}?group_id={{ $group_id }}');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Adv::STATUS_ON }}};
                    global.footer_ajax('{{ url('adv/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Adv::STATUS_OFF }}};
                    global.footer_ajax('{{ url('adv/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('adv/delete') }}', post_data);
                    });
                }
            };
