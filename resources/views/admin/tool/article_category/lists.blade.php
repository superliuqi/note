    ...
    <script>
        layui.config({
            ...
            var search_active = {
                ...
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('article_category/add') }}');
                }
            };
            ...
            form.on('switch(status_btn)', function(obj){
                ...
                global.footer_ajax('{{ url('article_category/status') }}', post_data, false);
            });
            ...
            $('.layui-table .layui-btn').on('click', function (){
                ...
                if (event == 'add_category'){
                    global.layer_show('添加', '{{ url('article_category/add') }}?parent_id=' + id);
                } else if(event == 'edit') {
                    global.layer_show('编辑', '{{ url('article_category/edit') }}?id=' + id);
                } else if(event == 'del') {
                    layer.confirm('确定删除吗', function(index){
                        var post_data = {_token: csrf_token, id: id};
                        if (global.footer_ajax('{{ url('article_category/delete') }}', post_data, false)) {
