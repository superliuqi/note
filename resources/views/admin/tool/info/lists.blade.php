...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('info/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
            <th lay-data="{type:'checkbox'}"></th>
            <th lay-data="{field:'id', width:80, sort: true}">ID</th>
            <th lay-data="{field:'title',width:400}">标题</th>
            <th lay-data="{field:'image',toolbar:'#imgTpl'}">图片</th>
    ...
    <script type="text/html" id="statusTpl">
        <input type="checkbox" value="@{{d.id}}" lay-skin="switch" lay-text="{{ \App\Models\Info::STATUS_DESC[\App\Models\Info::STATUS_ON] }}|{{ \App\Models\Info::STATUS_DESC[\App\Models\Info::STATUS_OFF] }}" lay-filter="status_btn" @{{ d.status == 1 ? 'checked' : '' }}>
    </script>
@endsection
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('info/add') }}');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Info::STATUS_ON }}};
                    global.footer_ajax('{{ url('info/status') }}', post_data);
                },
                //锁定
                status_off: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\Info::STATUS_OFF }}};
                    global.footer_ajax('{{ url('info/status') }}', post_data);
                },
                //删除
                del: function(){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        global.footer_ajax('{{ url('info/delete') }}', post_data);
                    });
                }
            ...
            table.on('tool(row_list)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        ...
                        if (global.footer_ajax('{{ url('info/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                            layer.msg('删除成功');
                        }
                    ...
                    global.layer_show('编辑', '{{ url('info/edit') }}?id=' + data.id);
                }else if(obj.event === 'push'){
                    layer.confirm('确定推送吗', function(index){
                        var post_data = {_token: csrf_token, id: data.id};
                        if (global.footer_ajax('{{ url('info/push') }}', post_data, true)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
            ...
            form.on('switch(status_btn)', function(obj){
                ...
                global.footer_ajax('{{ url('info/status') }}', post_data, false);
            });

            //监听单元格编辑
            table.on('edit(row_list)', function(obj){
                var post_data = {_token: csrf_token, id: obj.data.id, position: obj.value};
                global.footer_ajax('{{ url('info/position') }}', post_data, false);
            });
        });
    </script>
@endsection
