...
@section('content')
    ...
    <table class="layui-table" lay-data="{url:'{{ url('yu_video/lists_ajax') }}', page:true, id:'row_list'}" lay-filter="row_list">
        <thead>
        <tr>
    ...
    <script>
        layui.config({
            ...
            var batch_active = {
                //添加按钮
                add: function(){
                    global.layer_show('添加', '{{ url('yu_video/add') }}');
                },
                //审核
                status_on: function(){
                    var post_data = {_token: csrf_token, status: {{ \App\Models\YuVideo::STATUS_ON }}};
                    global.footer_ajax('{{ url('yu_video/status') }}', post_data);
                },
                //锁定
                status_off: function(){
            ...
            table.on('tool(row_list)', function(obj){
                var data = obj.data;
                if(obj.event === 'edit'){
                    global.layer_show('编辑', '{{ url('yu_video/edit') }}?id=' + data.id);
                }else if(obj.event === 'del'){
                    layer.confirm('确定删除吗', function(index){
                        var post_data = {_token: csrf_token, id: data.id};
                        if (global.footer_ajax('{{ url('yu_video/delete') }}', post_data, false)) {
                            obj.del();
                            layer.close(index);
                        }
                    });
